...
@section('content')
  <div class="col-md-12">
    <div class="row no-gutters p-3 border rounded d-flex align-items-center flex-md-row mb-4 shadow-sm position-relative">
      ...
      <div class="col-auto d-none d-lg-block product-section-images">
        <img src="{{ asset('storage/' . $product->image) }}" class="product-section-thumbnail img-thumbnail" width="250" height="200" id="mainImage">
        <div class="mt-2">
          @if ($product->images)
            <img src="{{ asset('storage/' . $product->image) }}" class="img-thumbnail product-section-thumbnail" width="40" height="40">
            @foreach (json_decode($product->images, true) as $image)
              <img src="{{ asset('storage/' . $image) }}" width="40" height="40" class="img-thumbnail">
            @endforeach
          @endif
        </div>
      </div>
